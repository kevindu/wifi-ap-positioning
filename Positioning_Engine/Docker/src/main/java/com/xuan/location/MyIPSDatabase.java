 ...
 * @author Jiuzhou
 */
import com.andrea.indoor.Constants;
import com.andrea.indoor.ProbabilisticPositioning;
...
//    public static String radiomap;
    // Genoa
    static ProbabilisticPositioning probalisticPositioning = new ProbabilisticPositioning(Constants.RADIO_MAP_PATH + "/" + Constants.RADIO_MAP_NAME);
//    ProbabilisticPositioning probalisticPositioning = null;
    ...
    public MyIPSDatabase() {
        ...
        try {
//            probalisticPositioning = new ProbabilisticPositioning(radiomap);
            probalisticPositioning.load();
            System.out.println("RadioMap loaded successfully!");
        } catch (IOException ex) {
            System.out.println("RadioMap NOT FOUND!");
            Logger.getLogger(MyIPSDatabase.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    // get the RSS of the mobile device through device MAC address
       public int[] getRSSData2(String mac_addr) {
        int[] rss_data = {-110, -110, -110, -110, -110, -110, -110, -110};
        try {
            Statement s = con.createStatement();
    ...
    // 18/06/2017 Genoa algorithm
    public Point ProbilisticPos(int[] measuredRSS) {
        double[] _rssScan = new double[measuredRSS.length];
        for(int i=0; i<measuredRSS.length; i++) {
            _rssScan[i] = measuredRSS[i];
}
        double[] coo = new double[2];
        try {
            coo = probalisticPositioning.getPosition(_rssScan, 6, true);
        } catch (IOException ex) {
            Logger.getLogger(MyIPSDatabase.class.getName()).log(Level.SEVERE, null, ex);
        }

        return new Point((int)coo[0], (int)coo[1]);
    }
    ...
    public Point KNN(int[] measuredRSS) {
        ...
        try {
            ...
            if (resultList.get(0).euclidean_dist <= 33000) {//30
    ...
    public void setAPPosition(String dev_mac_addr, String x, String y) throws SQLException {
     ...
     public void saveData(int x1, int y1, int x2, int y2) throws SQLException {
        ...
        String query = "INSERT INTO data(actual_position, estimated_position) "
                + "VALUES(POINT(?,?),POINT(?,?))";
